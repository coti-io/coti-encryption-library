...
class Wallet extends MonitoringProvider {
    ...
    async autoDiscoverAddressesForSeed(addressGap) {
        const addresses = await walletUtils.getAddressesOfWallet(this, addressGap);
        console.log(`${addresses.length} address(es) found at fullnode`);
        if(addresses.length > 0) await this.checkBalancesOfAddresses(addresses); // call to get addresses details
