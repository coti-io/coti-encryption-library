...
const walletAddressesInitalValues = [ [], [], 0, false ];
...
const self = module.exports = {
  ...
  getAddressesOfWallet: async wallet => {
    let [addressesToCheck, addressesThatExists, NextChunk, notExistsAddressFound] = walletAddressesInitalValues;
  ...
  getFullNodeFees: async (Wallet, amountToTransfer, feeIncluded = false) => {
    try {
      ...
      const res = await axios.put(`${FULL_NODE_URL}/fee`, {'originalAmount' : amountToTransfer, feeIncluded, userHash, userSignature});
  ...
  getNetworkFees: async (fullNodeFee, userHash, feeIncluded = false) => {
    try {
      const res = await axios.put(`${TRUSTSCORE_URL}/networkFee`, { 'fullNodeFeeData' : fullNodeFee , feeIncluded, userHash});
  ...
  createMiniConsensus: async ({userHash, fullNodeFee, networkFee, feeIncluded}) => {
    const iteration = 3;
    feeIncluded = feeIncluded ? true : false;
