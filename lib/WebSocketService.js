...
function openWebSocketConnection(monitoringProvider, successCallback, reconnectFailedCallback) {
  ...
  const addrs = this._MonitoringProvider.getWalletAddressHexes();
    ...
    let balanceSubscription = this.Client.subscribe(`/topic/${addressHex}`, async ({body}) => {
      try {
        ...
        if(body.message === 'Balance Updated!'){
          const address = this._MonitoringProvider.getWalletAddresses().get(body.addressHash);
...
function addressPropagationSubscriber(address) {
  ...
  let addressPropagationSubscription = this._MonitoringProvider.subscribe(`/topic/address/${addressHex}`, ({body}) => {
    try {
      body = JSON.parse(body);
