...
module.exports = {
    initSocketConnection: async (Wallet, addresses) => await openWebSocketConnection(Wallet, addresses),
}
...
const openWebSocketConnection = async (Wallet, addresses) => {
    _Wallet = Object.create(Wallet);
    ...
    Client.connect({}, () => {
      ...
      onConnected(addresses)
        ...
        console.log(error)
        reconnect(socketUrl, successCallback, addresses);
    });
...
const onConnected = addresses => {
  console.log('addresses: ', addresses)
  addresses = addresses == null || addresses == undefined ? new Array() : addresses ;

  addresses.length && addresses.forEach(async address => await connectToAddress(address));
...
const connectToAddress = address => {
  ...
  let transactionSubscription = Client.subscribe(`/topic/addressTransactions/${address}`, async ({body}) => {
    try {
      body = JSON.parse(body);
      _Wallet.onReceivedTransaction(body);
    } catch (error) {
      console.log(error);
    };
  });
...
const checkBalanceAndSubscribeNewAddress = async (address) => {
  ...
  const nextAddressPropogation = _Wallet.generateAddressByIndex( nextAddressIndex + 1 ).getAddressHex();
