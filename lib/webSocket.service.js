    ...
    try {
      ...
      if(body.message == 'Balance Updated!'){
        ...
        if(addressIndex === undefined) {
          const errorMsg = `Error - Address index not found for addressHex:${body.addressHash}`;
          console.log(errorMsg);
          throw new Error(errorMsg);
        }
        const { balance, preBalance } = body;
        setAddressWithBalance(balance === null ? 0 : balance, preBalance === null ? 0 : preBalance, addressIndex);
      }
    } catch (error) {
      console.log(error);
    };
...
const checkBalanceAndSubscribeNewAddress = async (address) => {
  ...
  const addressIndex =  _Wallet.getWalletAddresses(address).index;
  console.log(`subscribing for address generation: ${address} idx:${addressIndex}`);
  connectToAddress(address);
...
const setAddressWithBalance = (addressBalance, addressPreBalance, nextAddressIndex) => {
  ...
  console.log(`Setting balance for next address index:${nextAddressIndex}`);
