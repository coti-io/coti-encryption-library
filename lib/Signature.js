...
class FullnodeFeeSignatue extends Signature {
    ...
    getBytes() {
        let arr = [];
        const amountInBytes = stringUtils.getBytesFromString(this.removeZerosFromEndOfNumber(this.amount.toString()));
...
class OpenDispute extends Signature {
    ...
    getBytes() {
        ...
        this.items.forEach(item => {
            ...
            let id = cryptoUtils.numberToByteArray(Number(this.removeZerosFromEndOfNumber(item.id.toString())), 8);
...
class UploadComments extends Signature {
    ...
    getBytes() {
        ...
        this.itemIds.forEach(itemId => {
            let id = cryptoUtils.numberToByteArray(Number(this.removeZerosFromEndOfNumber(itemId.toString())), 8);
            arr = arr.concat(id);
        });
...
class GetDisputeDetails extends Signature {
    ...
    getBytes() {
        let arr = [];
        arr = arr.concat(stringUtils.hexToBytes(this.disputeHash));
        let id = cryptoUtils.numberToByteArray(Number(this.removeZerosFromEndOfNumber(this.itemId.toString())), 8);
        arr = arr.concat(id);
        return arr;
    }
...
class GetUnreadNotifications extends Signature {
    ...
    getBytes() {
        let arr = [];
        let timeInBytes = cryptoUtils.numberToByteArray(Number(this.removeZerosFromEndOfNumber(this.creationTime)),8);
